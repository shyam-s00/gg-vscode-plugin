...
exports.HTTP_FILE_EXTENSIONS = void 0;
exports.parseHttpRequests = parseHttpRequests;
/** File extensions gg treats as `.http`-dialect request files. */
exports.HTTP_FILE_EXTENSIONS = new Set(['.http', '.rest']);
// ─────────────────────────────────────────────────────────────────────────────
// Internal patterns
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * command.
 */
function parseHttpRequests(text) {
...
function parseBlock(lines, lineStart, lineEnd, name) {
    let requestLine = -1;
    let method = '';
    let url = '';
    const headers = {};
    ...
    for (let i = contentStart; i <= lineEnd; i++) {
        ...
        if (exportMatch) {
            exports.push({
                ...
                engine: exportMatch[2].toLowerCase(),
...
function parseRequestLine(trimmed) {
