...
exports.parseAssertResult = parseAssertResult;
exports.buildAssertArgs = buildAssertArgs;
exports.handleAssert = handleAssert;
exports.buildAssertResultHtml = buildAssertResultHtml;
 ...
 */
function parseAssertResult(stdout) {
...
function buildAssertArgs(baseline, compare, opts) {
    const args = [
...
async function promptAssertOptions() {
 ...
 */
async function handleAssert(configMgr, installer, a, b) {
    ...
    let result;
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Gopher-Glide: Running assertion…', cancellable: false }, async () => {
        ...
        if (output.code !== 0 && output.code !== 1) {
            ...
            vscode.window.showErrorMessage(`Gopher-Glide: gg snap assert failed (exit ${output.code}): ${output.stderr.trim() || output.stdout.trim()}`);
            return;
        }
    ...
    const panel = (0, snapDetailPanel_1.getSnapDetailPanel)('Assert Result');
    panel.webview.html = buildAssertResultHtml(baseline, compare, result, opts);
}
// ─────────────────────────────────────────────────────────────────────────────
// HTML — exported for smoke-testing
// ─────────────────────────────────────────────────────────────────────────────
function buildAssertResultHtml(baseline, compare, result, opts) {
...
//# sourceMappingURL=snapAssertPanel.js.map