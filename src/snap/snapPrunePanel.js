...
exports.parsePruneResult = parsePruneResult;
exports.buildPruneArgs = buildPruneArgs;
exports.hasAtLeastOneFilter = hasAtLeastOneFilter;
exports.handlePrune = handlePrune;
exports.buildPruneResultHtml = buildPruneResultHtml;
const vscode = __importStar(require("vscode"));
const snapCliRunner_1 = require("./snapCliRunner");
 ...
 */
function parsePruneResult(stdout) {
...
function buildPruneArgs(snapDir, opts) {
...
function hasAtLeastOneFilter(opts) {
 ...
 */
async function promptPruneOptions(prefilledIds) {
    ...
    if (!hasAtLeastOneFilter(opts)) {
        vscode.window.showErrorMessage('Gopher-Glide: At least one filter (--ids, --keep-last, --older-than, or --tag) is required.');
        return undefined;
    }
    return opts;
}
 ...
 */
async function handlePrune(configMgr, installer, snapDir, prefilledIds, onRefresh) {
    ...
    await vscode.window.withProgress({
        ...
        const output = await (0, snapCliRunner_1.runSnapCommand)(binPath, args);
        if (output.code !== 0) {
            vscode.window.showErrorMessage(`Gopher-Glide: gg snap prune failed (exit ${output.code}): ${output.stderr.trim() || output.stdout.trim()}`);
            return;
        }
    ...
    const panel = (0, snapDetailPanel_1.getSnapDetailPanel)(result.dry_run ? 'Prune Preview' : 'Prune Complete');
    panel.webview.html = buildPruneResultHtml(result, opts);
}
// ─────────────────────────────────────────────────────────────────────────────
// HTML — exported for smoke-testing
// ─────────────────────────────────────────────────────────────────────────────
function buildPruneResultHtml(result, opts) {
    ...
    const filtersUsed = [
        ...
        .filter(Boolean)
        .join('  ·  ');
    const candidateRows = result.candidates.map((c) => `<tr>
      <td>${c.id ?? '—'}</td>
      <td>${escHtml(c.tag ?? '')}</td>
...
//# sourceMappingURL=snapPrunePanel.js.map