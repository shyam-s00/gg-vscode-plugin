...
const snapAssertPanel_1 = require("../snap/snapAssertPanel");
// ─────────────────────────────────────────────────────────────────────────────
// Fixtures
// ─────────────────────────────────────────────────────────────────────────────
const DEFAULT_OPTS = {
...
function makeSnap(tag, filePath) {
    return {
        version: 1,
...
const PASSED_RESULT = { passed: true, violations: [] };
const FAILED_RESULT = {
    passed: false,
    violations: [
...
suite('parseAssertResult', () => {
    test('parses a passed result with no violations', () => {
        ...
        const result = (0, snapAssertPanel_1.parseAssertResult)(json);
        assert.ok(result);
    ...
    test('returns undefined for empty stdout', () => {
        assert.strictEqual((0, snapAssertPanel_1.parseAssertResult)(''), undefined);
        assert.strictEqual((0, snapAssertPanel_1.parseAssertResult)('  '), undefined);
    });
    test('returns undefined for invalid JSON', () => {
        assert.strictEqual((0, snapAssertPanel_1.parseAssertResult)('not json'), undefined);
    });
    test('returns undefined when passed field is missing', () => {
        const json = JSON.stringify({ violations: [] });
        assert.strictEqual((0, snapAssertPanel_1.parseAssertResult)(json), undefined);
    });
...
suite('buildAssertArgs', () => {
    test('builds the minimal args from default options', () => {
        const args = (0, snapAssertPanel_1.buildAssertArgs)(BASELINE, COMPARE, DEFAULT_OPTS);
    ...
    test('includes --deny-removed-fields when set', () => {
        const args = (0, snapAssertPanel_1.buildAssertArgs)(BASELINE, COMPARE, { ...DEFAULT_OPTS, denyRemovedFields: true });
        assert.ok(args.includes('--deny-removed-fields'));
    });
    test('includes --fail-on-warn when set', () => {
        const args = (0, snapAssertPanel_1.buildAssertArgs)(BASELINE, COMPARE, { ...DEFAULT_OPTS, failOnWarn: true });
...
suite('buildAssertResultHtml', () => {
    test('shows "passed" content for a clean result', () => {
        const html = (0, snapAssertPanel_1.buildAssertResultHtml)(BASELINE, COMPARE, PASSED_RESULT, DEFAULT_OPTS);
        assert.ok(html.includes('passed'));
        assert.ok(!html.includes('REGRESSION'));
    });
    test('shows violation rows for a failed result', () => {
        const html = (0, snapAssertPanel_1.buildAssertResultHtml)(BASELINE, COMPARE, FAILED_RESULT, DEFAULT_OPTS);
        assert.ok(html.includes('REGRESSION'));
    ...
    test('HTML-escapes tag names containing special chars', () => {
        ...
        const html = (0, snapAssertPanel_1.buildAssertResultHtml)(snap, COMPARE, PASSED_RESULT, DEFAULT_OPTS);
        assert.ok(!html.includes('<b>injection</b>'));
        assert.ok(html.includes('&lt;b&gt;injection&lt;/b&gt;'));
    });
});
//# sourceMappingURL=snapAssert.test.js.map