...
function makeEndpoint(id: string, p99: number, errorRate = 0, sampleCount = 100) {
  return {
    id,
...
function makeSnap(tag: string, startTime: string, endpoints = [makeEndpoint('GET:http://localhost/api', 100)]): LoadedSnap {
  return {
    version: 1,
...
suite('formatEndpointId', () => {
  test('extracts method and path from a full URL', () => {
    assert.strictEqual(formatEndpointId('GET:http://localhost:8080/api/users'), 'GET /api/users');
  });

  test('includes query string in the formatted label', () => {
    assert.strictEqual(formatEndpointId('GET:http://localhost/search?q=foo'), 'GET /search?q=foo');
  });

  test('returns raw value when URL is not parseable', () => {
    assert.strictEqual(formatEndpointId('GET:/relative/path'), 'GET /relative/path');
  });

  test('returns the id unchanged when there is no colon separator', () => {
    assert.strictEqual(formatEndpointId('no-colon-here'), 'no-colon-here');
  });

  test('handles POST with a full URL', () => {
    assert.strictEqual(formatEndpointId('POST:http://api.example.com/v2/orders'), 'POST /v2/orders');
  });
});
...
suite('computeDiff', () => {
  ...
  test('marks high error_rate delta as REGRESSION even if latency is fine', () => {
    ...
    const diffs = computeDiff(base, cmp);
  ...
  test('returns an empty array when both snaps have no endpoints', () => {
    ...
    assert.deepStrictEqual(computeDiff(base, cmp), []);
  });
  ...
  test('always orders oldest as baseline when inputs are swapped', () => {
    ...
    const diffs = computeDiff(COMPARE, BASELINE);
