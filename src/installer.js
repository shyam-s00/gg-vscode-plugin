...
// Binary helpers
// ─────────────────────────────────────────────────────────────────────────────
function execBinary(bin, args) {
    return new Promise((resolve, reject) => childProcess.execFile(bin, args, { timeout: 8_000 }, (err, stdout) => err ? reject(err) : resolve(stdout.trim())));
}
/**
...
class Installer {
    ...
    constructor(context, configMgr) {
        ...
        this._storageBinPath = path.join(storageDir, `${BINARY_NAME}${this._platform.binExt}`);
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                if (latest && installed !== latest) {
                    ...
                    vscode.window.showInformationMessage(`Gopher-Glide: gg is up to date (${installed || release.tag_name}). ✅`);
                }
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                if (!silent) {
                    vscode.window.showWarningMessage(`Gopher-Glide: Could not check for updates — ${msg}`);
                }
                else {
    ...
    async _isBinaryAvailable(binPath) {
        try {
    ...
    async _getInstalledVersion(binPath) {
        try {
    ...
    async _install({ reason, release, }) {
        ...
        if (installationMode === 'manual') {
            ...
            const choice = await vscode.window.showInformationMessage(`Gopher-Glide: ${prompt}`, 'Download', 'Not Now');
            if (choice !== 'Download') {
                return;
            }
        }
     ...
     */
    async _notifyBinaryMissing() {
        if (this.context.globalState.get(DONT_ASK_MISSING_BINARY_KEY, false)) {
            return;
        }
        const choice = await vscode.window.showWarningMessage('Gopher-Glide: gg binary not found. Download it now, or point the extension at an existing install.', 'Download Now', 'Set Custom Path…', "Don't ask again");
    ...
    async _performInstall(release) {
        try {
            ...
            if (!asset) {
                vscode.window.showErrorMessage(`Gopher-Glide: No release asset found for your platform ` +
            ...
            vscode.window.showInformationMessage(`Gopher-Glide: gg ${resolvedRelease.tag_name} installed successfully! 🚀`);
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            vscode.window.showErrorMessage(`Gopher-Glide: Installation failed — ${msg}`);
        }
    }
    /** Shows a user-facing notification for an available update. */
    async _promptUpdate({ installed, latest, release, }) {
        const choice = await vscode.window.showInformationMessage(`Gopher-Glide: gg ${latest} is available (you have ${installed}).`, 'Update Now', 'Release Notes', 'Skip');
    ...
    _archivePath(tagName) {
        return `${this._storageBinPath}-${tagName}${this._platform.archiveExt}`;
    }
    async _downloadWithProgress(asset, archivePath, tagName) {
        await vscode.window.withProgress({
    ...
    _extractArchive(archivePath) {
        ...
        return new Promise((resolve, reject) => childProcess.execFile(cmd, args, { timeout: 30_000 }, (err, _stdout, stderr) => {
            if (err) {
